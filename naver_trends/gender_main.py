    ...
    for client in client_info_list:
        ...
        for point in range(0, sheet_size, MAX_ANAL_BATCH_SIZE):
            ...
            for idx, row in enumerate(sheet_chunk_list):
                male_data_size = len(keyword_dict[keyword_list[idx]]['dmc'])
                if male_data_size > 0:
                ...
                female_data_size = len(keyword_dict[keyword_list[idx]]['dfc'])
                if female_data_size > 0:
                    row['date'] = keyword_dict[keyword_list[idx]]['dfc'].keys()
                    row['queries'] = keyword_dict[keyword_list[idx]]['dfc'].values()
                    row['gender'] = '여'
                    dataframe_list.append(pd.DataFrame(columns=dataframe_cols, data=sheet_chunk_list[idx]))
                dataframe_rows = dataframe_rows + male_data_size + female_data_size
